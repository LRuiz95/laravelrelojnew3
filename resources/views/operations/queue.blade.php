...
<div class="card shadow-sm" data-sync-queue
    data-url="{{ route('operations.queue.data', request()->query()) }}"
    data-cancel-url="{{ url('/sync-queue') }}">
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0 table-cards">
            <thead><tr><th>Dispositivo</th><th>Operación</th><th>Estado</th><th>Progreso</th><th>Resultado</th><th>Fecha</th><th>Acciones</th></tr></thead>
            ...
            @forelse ($syncs as $sync)
                ...
                <tr data-sync-id="{{ $sync->id }}">
                    <td data-label="Dispositivo"><a href="{{ route('devices.show', $sync->device) }}" class="fw-semibold text-decoration-none">{{ $sync->device->name }}</a></td>
            ...
            @empty
                <tr><td colspan="7" class="text-center text-muted py-5"><i class="bi bi-inbox fs-2 d-block mb-2"></i>No hay operaciones en la cola.</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
...
<script>
    (() => {
        ...
        const render = (syncs) => {
            if (!syncs.length) {
                rows.innerHTML = '<tr><td colspan="7" class="text-center text-muted py-5"><i class="bi bi-inbox fs-2 d-block mb-2"></i>No hay operaciones en la cola.</td></tr>';
                return;
            }
            rows.innerHTML = syncs.map((sync) => {
                ...
                const action = active
                    ? `<button class="btn btn-sm btn-outline-warning" data-cancel="${sync.id}" title="Cancelar"><i class="bi bi-stop-circle"></i></button>`
                    : (sync.retry_url ? `<button class="btn btn-sm btn-outline-warning" data-retry="${sync.id}" title="Reintentar solo este dispositivo"><i class="bi bi-arrow-repeat"></i></button>`
                    : `<button class="btn btn-sm btn-icon-danger" data-delete="${sync.id}" title="Eliminar"><i class="bi bi-trash"></i></button>`)
                ...
                const subStage = (sync.operation === 'all' && stageNames[sync.stage])
                    ...
                    : '';
                return `<tr data-sync-id="${sync.id}">
                    <td data-label="Dispositivo">${sync.device_url ? `<a href="${sync.device_url}" class="fw-semibold text-decoration-none">${esc(sync.device)}</a>` : esc(sync.device)}</td>
        ...
        rows.addEventListener('click', async (event) => {
            ...
            const id = cancel?.dataset.cancel || remove?.dataset.delete || retry?.dataset.retry;
            if (!id) return;
            if (cancel && !window.confirm('¿Cancelar esta sincronización?')) return;
            if (remove && !window.confirm('¿Eliminar este registro de la cola?')) return;
            if (retry && !window.confirm('¿Reintentar solo este dispositivo?')) return;
            try {
                const data = await request(`${queue.dataset.cancelUrl}/${id}${cancel ? '/cancel' : (retry ? '/retry' : '')}`, cancel || retry ? 'POST' : 'DELETE');
                window.dashToast?.({ type: 'success', title: cancel ? 'Sincronización cancelada' : (retry ? 'Reintento enviado' : 'Registro eliminado'), message: data.message });
                refresh();
            } catch (error) {
