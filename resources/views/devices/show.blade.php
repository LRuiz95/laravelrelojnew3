...
<div class="d-flex justify-content-between align-items-end flex-wrap gap-3 mb-4">
    <div>
        <div class="d-flex align-items-center gap-2 mb-2">
            ...
            <a href="{{ route('devices.index') }}" class="ref-chip text-decoration-none d-inline-flex">Dispositivos</a>
            <i class="bi bi-chevron-right small text-tertiary-token"></i>
            <span class="small text-tertiary-token">Dispositivo {{ $device->name }}</span>
        </div>
        <h2 class="h4 fw-bold mb-1 d-flex align-items-center gap-2 flex-wrap">
            {{ $device->name }}
            <span class="badge badge-with-dot {{ $statusCat }}">{{ $statusLabel }}</span>
        </h2>
        <p class="text-muted mb-0">
            <code>{{ $device->ip }}:{{ $device->port }}</code>
            @if ($device->serial_number)
                <span class="mono text-tertiary-token">· {{ $device->serial_number }}</span>
            @endif
        </p>
    </div>
        ...
        <div class="card-body py-3">
            <div class="row g-3">
                <div class="col-6 col-md">
                    ...
                    <div class="fw-semibold">{{ $info['device_name'] ?? '-' }}</div>
                </div>
                <div class="col-6 col-md">
                    <div class="small text-tertiary-token mb-1"><i class="bi bi-cpu me-1"></i>Firmware</div>
                    <div class="fw-semibold">{{ $info['vendor'] ?? '-' }} <span class="text-tertiary-token">·</span> {{ $info['version'] ?? '-' }}</div>
                </div>
                <div class="col-6 col-md">
                    <div class="small text-tertiary-token mb-1"><i class="bi bi-clock me-1"></i>Hora del equipo</div>
                    <div class="fw-semibold mono">{{ $info['time'] ?? '-' }}</div>
                </div>
                <div class="col-6 col-md">
            ...
            <div class="card-body py-3">
                <div class="d-flex justify-content-between align-items-start gap-3 mb-3">
                    ...
                    <div class="text-end">
                        ...
                        <span class="badge badge-with-dot {{ ($syncBadgeMeta[$ls?->status] ?? ['cat-gray'])[0] }}">
                            {{ $ls ? \Illuminate\Support\Str::ucfirst($ls->status) : 'Nunca ejecutada' }}
                        </span>
                    </div>
                </div>

                <div class="alert alert-secondary d-flex justify-content-between align-items-center gap-3 py-2 px-3 d-none"
                     id="sync-status" data-url="{{ route('devices.sync-status', $device) }}">
                    <span>
        ...
        <div class="card shadow-sm h-100">
            ...
            <div class="card-body py-2" data-recent-syncs>
                @forelse ($recentSyncs as $sync)
                    @php [$syncCat, $syncIcon] = $syncBadgeMeta[$sync->status] ?? ['cat-gray', 'bi-clock']; @endphp
                    <div class="d-flex align-items-center gap-3 py-2">
                        <span class="badge {{ $syncCat }}"><i class="bi {{ $syncIcon }}"></i></span>
                                    ...
                                    @forelse ($attendances as $attendance)
                                        <tr>
                                            ...
                                            <td data-label="Marcado">
                                                <span class="badge badge-with-dot {{ $attendance->stateColorClass() }}">{{ $attendance->shortStateLabel() }}</span>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
    ...
    <div class="col-lg-4">
        <div class="card shadow-sm h-100">
            <div class="card-header d-flex justify-content-between align-items-center py-3">
                ...
                <a href="{{ route('attendances.index', ['device_id' => $device->id]) }}" class="btn btn-sm btn-ghost">
            ...
            <div class="card-body py-2" data-recent-feed>
                @forelse ($attendances->take(6) as $attendance)
                    <div class="d-flex align-items-center gap-2 py-2">
                        <span class="avatar is-sm">{{ strtoupper(substr($attendance->employee?->name ?? '?', 0, 1)) }}</span>
                        <div class="flex-grow-1">
                        ...
                        <div class="text-end">
                            <div><span class="badge badge-with-dot {{ $attendance->stateColorClass() }}">{{ $attendance->shortStateLabel() }}</span></div>
